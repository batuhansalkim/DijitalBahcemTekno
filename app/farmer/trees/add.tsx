...
export default function AddTreeScreen() {
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      // Sadece RFID ve konum bilgilerini göster
      Alert.alert(
        'Ağaç Bilgileri Kaydedildi! ✅',
        `RFID UID: ${form.rfidCode}\n\nKonum:\nEnlem: ${form.location.latitude}\nBoylam: ${form.location.longitude}\n\nAğaç ID: ${form.treeId}`,
        [
          {
            text: 'Tamam',
            onPress: () => {
